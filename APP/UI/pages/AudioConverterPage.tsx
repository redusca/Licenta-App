...
import {
    ArrowLeft, Music, ChevronRight, HardDrive, FolderOpen,
...
function FolderBrowser({
    ...
    const audioEntries = entries.filter(e => !e.is_dir);
    const allAudioSelected = audioEntries.length > 0 && audioEntries.every(e => selected.has(e.path));
    const someAudioSelected = audioEntries.some(e => selected.has(e.path));

    const toggleAll = () => {
        if (allAudioSelected) {
            setSelected(new Set());
        } else {
            setSelected(new Set(audioEntries.map(e => e.path)));
        }
    };
                            ...
                            {audioEntries.length > 0 && (
                                <div className="flex items-center gap-3 px-4 py-2 mb-1 border-b border-slate-200 dark:border-slate-800">
                                    <button type="button" onClick={toggleAll}
                                        className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${allAudioSelected
                                            ? 'bg-blue-500 border-blue-500 text-white'
                                            : someAudioSelected
                                                ? 'bg-blue-500/30 border-blue-400 text-white'
                                                : 'border-slate-400 hover:border-slate-300'
                                            }`}>
                                        {allAudioSelected ? <Check className="w-3 h-3" /> : someAudioSelected ? <Minus className="w-3 h-3" /> : null}
                                    </button>
                                    <span className="text-xs text-slate-500">{audioEntries.length} audio file{audioEntries.length !== 1 ? 's' : ''} — {selected.size} selected</span>
                                </div>
                            )}
...
export const AudioConverterPage: React.FC = () => {
    ...
    // Browse a folder and add all audio in it
    const browseFolder = async () => {
        ...
        try {
            ...
            const data = await res.json();
            const audioFiles: FileItem[] = (data.files || [])
                .filter((f: any) => !f.is_dir && isAudioFile(f.name))
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
                        ...
                        {files.length === 0 && (
                            <div className="flex flex-col items-center gap-3 py-12 mt-4 border-2 border-dashed border-slate-700 rounded-xl text-center">
                                ...
                                <p className="text-xs text-slate-600">Use the buttons above to browse for audio files or pick from a virtual drive.</p>
                            </div>
                        )}
                    </div>
